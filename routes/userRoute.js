...
router.route('/').put(upload.single('profilePic'), [
  body('fname').if(body('fname').exists()).trim().isLength({max: 50}).escape(),
  body('lname').if(body('lname').exists()).trim().isLength({max: 50}).escape(),
...
router.route('/search/:name').get(userController.usersByName);
